...
import {
  ...
  RefreshCw, ExternalLink, Sparkles, Heart, MapPin, Clock, X,
} from 'lucide-react';

type ListType = 'featured' | 'popular';
...
const TAB_DEFS: { key: ListType; label: string; icon: React.ReactNode; hint: string }[] = [
  { key: 'featured', label: 'Featured Tours',  icon: <Sparkles size={13} />, hint: 'Shown in Col 1 of the navbar Tours dropdown (max 4)' },
  { key: 'popular',  label: 'Most Popular',    icon: <Heart size={13} />,    hint: 'Shown in Col 4 of the navbar Tours dropdown (max 4)' },
];

export default function NavDropdownPage() {
  const [slots, setSlots]   = useState<DropdownSlot[]>([]);
  const [tours, setTours]   = useState<Tour[]>([]);
  const [loading, setLoading] = useState(true);
  const [saving, setSaving]   = useState(false);
  const [activeTab, setActiveTab] = useState<ListType>('featured');
  ...
  async function load() {
    ...
    const [slotsRes, toursRes] = await Promise.all([
      supabase.from('nav_dropdown_tours').select('*').order('sort_order'),
    ...
    setSlots((slotsRes.data ?? []) as DropdownSlot[]);
  ...
  const tabSlots = useMemo(() =>
    slots
      .filter(s => s.list_type === activeTab)
      .sort((a, b) => a.sort_order - b.sort_order)
      .map(s => ({ slot: s, tour: tours.find(t => t.id === s.tour_id) }))
      .filter(x => x.tour),
  [slots, tours, activeTab]);

  const tabSlotIds = new Set(slots.filter(s => s.list_type === activeTab).map(s => s.tour_id));
  ...
  }, [tours, tabSlotIds, search, cityFilter, activeTab]);

  async function addTour(tour: Tour) {
    const existing = slots.filter(s => s.list_type === activeTab);
    const maxOrder = existing.length > 0 ? Math.max(...existing.map(s => s.sort_order)) : -1;
    const { data, error } = await supabase
      .from('nav_dropdown_tours')
      .insert({ list_type: activeTab, tour_id: tour.id, sort_order: maxOrder + 1 })
      .select()
      .single();
    if (error) { toast.error(error.message); return; }
    setSlots(prev => [...prev, data as DropdownSlot]);
    toast.success(`"${tour.title}" added to ${activeTab}`);
  }

  async function removeTour(slot: DropdownSlot, title: string) {
    const { error } = await supabase.from('nav_dropdown_tours').delete().eq('id', slot.id);
    if (error) { toast.error(error.message); return; }
    setSlots(prev => prev.filter(s => s.id !== slot.id));
    toast.success(`"${title}" removed`);
  }

  async function move(slot: DropdownSlot, dir: 'up' | 'down') {
    const sorted = slots
      .filter(s => s.list_type === activeTab)
      .sort((a, b) => a.sort_order - b.sort_order);
    ...
    setSaving(true);
    await Promise.all([
      supabase.from('nav_dropdown_tours').update({ sort_order: other.sort_order }).eq('id', slot.id),
      supabase.from('nav_dropdown_tours').update({ sort_order: slot.sort_order }).eq('id', other.id),
    ]);
  ...
  return (
    <div className="page">
      ...
      <div className="page-header">
        <div>
          <h1 className="page-title">Tours Dropdown Menu</h1>
          <p className="page-subtitle">
            Curate the Featured &amp; Popular columns shown in the navbar mega menu
          </p>
        </div>
      ...
      <div className="tabs" style={{ marginBottom: 20 }}>
        {TAB_DEFS.map(tab => (
          <button
            key={tab.key}
            onClick={() => { setActiveTab(tab.key); setSearch(''); setCityFilter(''); }}
            className={`tab ${activeTab === tab.key ? 'tab-active' : ''}`}
          >
            <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              {tab.icon} {tab.label}
              <span style={{ fontSize: 11, opacity: .6, fontWeight: 500 }}>
                ({slots.filter(s => s.list_type === tab.key).length})
              </span>
            </span>
          </button>
        ))}
      ...
      <p style={{ fontSize: 12, color: 'var(--text-3)', marginBottom: 20 }}>{activeDef.hint}</p>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: 24, alignItems: 'start' }}>
        ...
        <div>
          <h3 style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-2)', textTransform: 'uppercase', letterSpacing: '.07em', marginBottom: 12 }}>
            {activeDef.label} — {tabSlots.length} tours
          </h3>

          {tabSlots.length === 0 ? (
            <div className="card" style={{ textAlign: 'center', padding: 48, color: 'var(--text-3)' }}>
              {activeDef.icon}
              <p style={{ fontSize: 13, marginTop: 12 }}>No tours selected yet. Add some from the right panel.</p>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              {tabSlots.map(({ slot, tour }, idx) => (
                <div key={slot.id} className="card" style={{ padding: '10px 14px', display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 2, flexShrink: 0 }}>
                    ...
                    <button onClick={() => move(slot, 'down')} disabled={saving || idx === tabSlots.length - 1} className="btn-icon"
                      style={{ opacity: idx === tabSlots.length - 1 ? 0.2 : 1, padding: 2 }}>
                      <ChevronDown size={14} />
                    </button>
                  </div>
            ...
            <div style={{ maxHeight: 480, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 5 }}>
              {available.length === 0 ? (
                <p style={{ fontSize: 12, color: 'var(--text-3)', textAlign: 'center', padding: '16px 0' }}>
                  {tabSlotIds.size === tours.length ? 'All tours are already added.' : 'No matches.'}
                </p>
              ) : available.map(tour => (
